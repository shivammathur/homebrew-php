class PhpAT85Experimental < Formula
  ...
  def install
    ...
    pgo_script = buildpath/"pgo-train.php"
    pgo_script.write <<~PHP
      <?php
      $records = [];
      for ($i = 0; $i < 6000; $i++) {
          $records[] = [
    ...
    # Build an instrumented binary, collect profile data, and then rebuild
    # with the generated profile.
    # Keep Linux on GCC-based PGO without LTO, as GCC LTO trips over PHP's
    # global register variables in opcache/JIT helper code.
    # Apply ThinLTO only on Apple Silicon and only for the final profiled
    # build, since Intel macOS runners can crash during the instrumented link.
